...
void mqnic_tx_write_prod_ptr(struct mqnic_ring *ring, u32 rank, u32 wqe) //u32 ->u16?
{
	iowrite32(MQNIC_QUEUE_CMD_SET_PROD_PTR | (ring->prod_ptr & MQNIC_QUEUE_PTR_MASK),
			ring->hw_addr + MQNIC_QUEUE_CTRL_STATUS_REG);	

    iowrite32(((rank & 0x1fff) << 3) | (wqe & 0x7),
            ring->hw_addr + MQNIC_QUEUE_RANK_WQE);
...
#endif
		dma_wmb();
		mqnic_tx_write_prod_ptr(ring,0x100,1);
	}
