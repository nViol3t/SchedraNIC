...
unsigned int mqnic_num_txq_entries = 256;
...
{
	//把 mqnic 设备绑定进内核 devlink 框架，让用户态能通过 devlink 工具管理和调试设备。?
	int ret = 0;
...
{
	int ret = 0;
	struct mqnic_dev *mqnic;// 自己定义的私有结构体，保存这个驱动所需的上下文
	struct devlink *devlink;// 后续把 NIC 注册到 devlink 框架
	struct device *dev = &pdev->dev;// Linux 通用设备模型
	struct pci_dev *bridge = pci_upstream_bridge(pdev);// 找到 pdev 的上游 PCI 桥（Bridge）设备
	...
	// 读取 PCIe 配置空间的 capability 信息（PCIe配置空间新增）：DEVCTL、LNKCAP 、LNKCTL 、LNKSTA 并打印		
	if (pdev->pcie_cap) { 
		u16 devctl;
		u32 lnkcap;
		u16 lnkctl;
	...
	mqnic = devlink_priv(devlink);// 每个 devlink 对象都会分配一块私有存储区域。拿到那块区域，当成 struct mqnic_dev 来用。
	mqnic->dev = dev;
	mqnic->pdev = pdev;
	pci_set_drvdata(pdev, mqnic);// 把私有数据 mqnic 挂到 pdev 上,“反向指针”机制
	...
	// Enable device
	// pci_enable_device_mem()只启用设备并请求内存资源（不请求I/O资源）设备只需要内存映射访问,即启用设备的 memory BAR（Base Address Register）
	// pci_enable_device()启用设备并请求所有资源（内存和I/O）
	...
	// 获取PCI设备三个BAR（Base Address Register）的空间信息和物理地址
	// 为什么是这三个？
	...
	// 为DMA启用总线控制 :只有 启用了总线主控的设备才能主动发起 DMA 访问
	pci_set_master(pdev);
