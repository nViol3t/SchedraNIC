module PIFO_SRAM_Level_1#(
   ...
   parameter RAMSTYLE = "distributed",
   //Don't Touch
...
(
   ...
   //Pop  to Child
   output   logic                                              Parents_Pop_req_valid  ,
   input    logic [SINGLE_DATA_WITHOUT_COUNTER-1:0]            Parents_Pop_resp_Data  ,
   //Addr Io
   ...
   localparam  FINISH_PIPE_DEPTH   = 'd1 ;
...
//   logic                               ram_rden       ;
   ...
   logic                                                 Pop_Finish_pipe_in    ,  Pop_Finish_pipe_out ;
   logic  [PUSH_REQ_PIPE_DEPTH-1:0]                      Pop_Finish_pipe_total ;
   ...
   logic                                                 Push_Finish_pipe_in    ,  Push_Finish_pipe_out ;
   logic  [PUSH_REQ_PIPE_DEPTH-1:0]                      Push_Finish_pipe_total ;
...
//      四级比较器，选择最小PTW值
   ...
   always_comb begin
      ...
      Top_Pop_req_ready_next         = 1'b0 ;//!Pop_Req_pipe_total && !Push_Req_pipe_total && !Push_Finish_pipe_total;
      ...
      //pop finish
      Pop_Finish_pipe_in             = 1'b0 ;
      //pop to child 
      Parents_Pop_req_valid_reg      = 'd0 ;
      Parents_Child_Addr_reg         = 'd0 ;
      //push req
      Push_Req_pipe_in               = 1'b0 ;
      Top_Push_ready_next            = 1'b0 ;//!Pop_Req_pipe_total && !Pop_Finish_pipe_total && !Push_Req_pipe_total && !Push_Finish_pipe_total;
      Push_Req_Data_pipe_in          =  'd0 ;
      //push req
      Parents_Push_valid_reg         = 1'b0 ;
      Parents_Push_Data_reg          = 'd0  ;
      //push finish
      Push_Finish_pipe_in            = 1'b0 ;
      //ram io 
      ...
      //pop req  
      // 第一级：接收Pop请求，读取SRAM
      if(Top_Pop_req_valid && !Pop_Req_pipe_total && !Push_Req_pipe_total && !Push_Finish_pipe_total && !Pop_Finish_pipe_total && !Pifo_Empty)begin
      ...
      //push req
      // 第一级：接收Push请求，读取SRAM
      else if(Top_Push_valid && !Pop_Req_pipe_total && !Push_Req_pipe_total && !Push_Finish_pipe_total && !Pop_Finish_pipe_total)begin//没有正在进行的 Pop 请求流水（!Pop_Req_pipe_total），并且上一次 Pop 响应已完成或空闲。
...
*/
   ...
   */
   ...
   #(
    .WORD_WIDTH      (1'b1),
    .PIPE_DEPTH      (FINISH_PIPE_DEPTH)
    // Don't set at instantiation
    //parameter                   TOTAL_WIDTH     = WORD_WIDTH * PIPE_DEPTH,
    // concatenation of each stage initial/reset value
    //parameter [TOTAL_WIDTH-1:0] RESET_VALUES    = 0
   )Push_Finish_Pipeline_inst
   (
      .clock         (clk),
      .clock_enable  (1'b1),
      .clear         (rst),
      .parallel_load (1'b0),
      .parallel_in   ({FINISH_PIPE_DEPTH{1'b0}}),  
      .parallel_out  (Push_Finish_pipe_total),
      .pipe_in       (Push_Finish_pipe_in),
      .pipe_out      (Push_Finish_pipe_out)
   );
   ...
   (
      ...
      .parallel_load (1'b0),
      .parallel_in   ({FINISH_PIPE_DEPTH{1'b0}}),  
      .parallel_out  (Pop_Finish_pipe_total),
      .pipe_in       (Pop_Finish_pipe_in),
      .pipe_out      (Pop_Finish_pipe_out)
   );
