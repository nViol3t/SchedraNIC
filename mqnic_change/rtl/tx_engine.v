...
always @* begin
    ...
    // 怎么改合适？添加流控？
    if(enable && active_desc_req_count_reg < MAX_DESC_REQ && !desc_table_active[desc_table_start_ptr_reg & DESC_PTR_MASK] && ($unsigned(desc_table_start_ptr_reg - desc_table_finish_ptr_reg) < DESC_TABLE_SIZE)) begin
        m_axis_tx_req_valid_next = 1'b1;
    end
    
    // wait transmit request from pifo then send descriptor fetch to desc_fetch
    s_axis_tx_resp_ready_next = enable && 
...
always @(posedge clk) begin
    ...
    // descriptor table operations 描述符表初始化
