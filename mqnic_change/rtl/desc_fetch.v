...
(
    ...
    parameter SEG_DATA_WIDTH = 64,//512
...
(
    ...
    input  wire [WQE_WIDTH-1:0]                   s_axis_req_wqe,
    ...
    //Descriptor dequeue request output to tx/rx_queue_manager
    output wire [PORTS*QUEUE_INDEX_WIDTH-1:0]     m_axis_desc_dequeue_req_queue,
    output wire [WQE_WIDTH-1:0]                   m_axis_desc_dequeue_req_wqe,
    ...
    //Descriptor dequeue response input from tx/rx_queue_manager
    ...
    input  wire [PORTS*WQE_WIDTH-1:0]             s_axis_desc_dequeue_resp_wqe,

    //Descriptor dequeue commit output to tx/rx_queue_manager
    ...
    input  wire [SEG_COUNT*SEG_BE_WIDTH-1:0]      dma_ram_wr_cmd_be,//128=2*64
...
reg [WQE_WIDTH-1:0] m_axis_desc_dequeue_req_wqe_reg = {WQE_WIDTH{1'b0}},m_axis_desc_dequeue_req_wqe_next;
...
reg [15:0] dma_read_desc_len_reg = 16'd0, dma_read_desc_len_next;
...
assign m_axis_desc_dequeue_req_wqe =  {PORTS{m_axis_desc_dequeue_req_wqe_reg}};
...
dma_psdpram_inst (
    ...
    //Read port 连接到 dma_client_axis_source的RAM接口
...
dma_client_axis_source_inst (
    ...
    //AXI stream read data output 连接到desc_fetch的data output
    ...
    // RAM interface 连接到dma_psdpram的RAM接口
...
reg s_axis_req_active_count_cond;
reg s_axis_req_table_free_cond;
reg s_axis_req_space_avail_cond;
reg s_axis_req_req_ready_cond;
always @* begin
    ...
    m_axis_desc_dequeue_req_wqe_next = m_axis_desc_dequeue_req_wqe_reg;
...
//    desc_table_wqe_en = 1'b0;
    desc_table_start_wqe = s_axis_desc_dequeue_resp_wqe[dequeue_resp_enc*WQE_WIDTH +: WQE_WIDTH];
    // queue query
    // wait for descriptor request

    s_axis_req_active_count_cond = (active_count_reg < DESC_TABLE_SIZE);
    s_axis_req_table_free_cond   = !desc_table_active[desc_table_start_ptr_reg & DESC_PTR_MASK];
    s_axis_req_space_avail_cond  = ($unsigned(desc_table_start_ptr_reg - desc_table_finish_ptr_reg) < DESC_TABLE_SIZE);
    s_axis_req_req_ready_cond    = (!m_axis_desc_dequeue_req_valid_reg || (m_axis_desc_dequeue_req_valid_reg & m_axis_desc_dequeue_req_ready));

    s_axis_req_ready_next = (enable 
                            && active_count_reg < DESC_TABLE_SIZE 
    ...
    if (dequeue_resp_enc_valid && !m_axis_dma_read_desc_valid_reg 
        ...
        // Calculate DMA length based on stored WQE count and block size 没有考虑WQE数量过多导致溢出 dyc修改
        m_axis_dma_read_desc_len_next = (DESC_SIZE * desc_table_start_wqe) << s_axis_desc_dequeue_resp_block_size[dequeue_resp_enc*LOG_BLOCK_SIZE_WIDTH +: LOG_BLOCK_SIZE_WIDTH];
    ...
    if (desc_table_active[desc_table_desc_read_ptr_reg & DESC_PTR_MASK] && desc_table_desc_read_ptr_reg != desc_table_start_ptr_reg) begin
        ...
        if (desc_table_desc_fetched[desc_table_desc_read_ptr_reg & DESC_PTR_MASK] 
            ...
            //dma_read_desc_len_next = (DESC_SIZE * desc_table_wqe[desc_table_desc_read_ptr_reg & DESC_PTR_MASK]) << desc_table_log_desc_block_size[desc_table_desc_read_ptr_reg & DESC_PTR_MASK];
...
always @(posedge clk) begin
    ...
    m_axis_desc_dequeue_req_wqe_reg <= m_axis_desc_dequeue_req_wqe_next;
    ...
    active_count_reg <= active_count_reg + inc_active - dec_active_1 - dec_active_2;//change inc_active

    if (desc_table_start_en) begin //need fix!!!
        for (i = 0; i < MAX_WQE; i = i + 1) begin
            if (i < desc_table_start_wqe) begin
              desc_table_active[(desc_table_start_ptr_reg + i) & DESC_PTR_MASK] <= 1'b1;
              desc_table_desc_fetched[(desc_table_start_ptr_reg + i) & DESC_PTR_MASK] <= 1'b0;
              desc_table_desc_read_done[(desc_table_start_ptr_reg + i) & DESC_PTR_MASK] <= 1'b0;
              desc_table_sel[(desc_table_start_ptr_reg + i) & DESC_PTR_MASK] <= desc_table_start_sel;
              desc_table_log_desc_block_size[(desc_table_start_ptr_reg + i) & DESC_PTR_MASK] <= desc_table_start_log_desc_block_size;
              desc_table_tag[(desc_table_start_ptr_reg + i) & DESC_PTR_MASK] <= {desc_table_start_tag[5], (desc_table_start_tag[4:0] + i[4:0]) & 5'h1F};
              desc_table_queue_op_tag[(desc_table_start_ptr_reg + i) & DESC_PTR_MASK] <= desc_table_start_queue_op_tag;
             end
        end
        desc_table_wqe[desc_table_start_ptr_reg & DESC_PTR_MASK] <= desc_table_start_wqe;
        desc_table_start_ptr_reg <= desc_table_start_ptr_reg + desc_table_start_wqe;
    end

    if (desc_table_desc_fetched_en) begin
    
        for (j = 0; j < MAX_WQE; j = j + 1) begin
            if (j < desc_table_wqe[desc_table_desc_fetched_ptr & DESC_PTR_MASK]) begin
                desc_table_desc_fetched[(desc_table_desc_fetched_ptr + j) & DESC_PTR_MASK] <= 1'b1;
            end
        end
    end

    if (desc_table_desc_read_en) begin
        //desc_table_desc_read_ptr_reg <= desc_table_desc_read_ptr_reg + desc_table_wqe[desc_table_desc_read_ptr_reg & DESC_PTR_MASK];
    ...
    if (desc_table_finish_en) begin
        ...
        // for (j = 0; j < desc_table_wqe[desc_table_finish_ptr_reg & DESC_PTR_MASK]; j = j + 1) begin
        //     desc_table_active[desc_table_finish_ptr_reg & DESC_PTR_MASK] <= 1'b0;
        // end
        // desc_table_finish_ptr_reg <= desc_table_finish_ptr_reg + desc_table_wqe[desc_table_finish_ptr_reg & DESC_PTR_MASK];
        desc_table_active[desc_table_finish_ptr_reg & DESC_PTR_MASK] <= 1'b0;
