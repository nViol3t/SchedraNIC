...
class Txq:
    ...
    async def write_prod_ptr(self, rank, wqe):
        await self.hw_regs.write_dword(MQNIC_QUEUE_RANK_WQE, rank << 16 | wqe )
        await self.hw_regs.write_dword(MQNIC_QUEUE_CTRL_STATUS_REG, MQNIC_QUEUE_CMD_SET_PROD_PTR | (self.prod_ptr & MQNIC_QUEUE_PTR_MASK))
...
class Interface:
    ...
    async def start_xmit(self, skb, tx_ring=None, csum_start=None, csum_offset=None,rank=0x1000,wqe=1):
        if not self.port_up:
            return
        ...
        ring.prod_ptr += wqe
        await ring.write_prod_ptr(rank,wqe)
