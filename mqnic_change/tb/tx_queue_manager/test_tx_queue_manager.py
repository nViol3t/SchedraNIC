...
async def run_test(dut):
    ...
    OP_TABLE_SIZE = 32  # 请根据你的设计确认
    MAX_INFLIGHT = 2    # 每个队列最多缓冲 2 个未完成项
    PTW = 16
    WQE_WITHE = 16
    # 初始化队列
    ...
    for i in range(50):
        ...
        for _ in range(random.randrange(8)):
            ...
            if queue_depth[q] < 16:
                ...
                wqe = 4
                await tb.axil_master.write_dword(q*32+MQNIC_QUEUE_RANK_WQE, rank << 16 | wqe )
                ...
                # 发送 PIFO 完成信号
                await tb.pifo_comp_source.send(PifoCompTransaction(queue=q))
