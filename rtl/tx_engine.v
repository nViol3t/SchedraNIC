...
(
    ...
    // Transmit response input (queue index)
    input  wire [QUEUE_INDEX_WIDTH-1:0]     s_axis_tx_resp_queue,
    input  wire [WQE_WIDTH-1:0]             s_axis_tx_resp_wqe, 
    input  wire [AXIS_TX_DEST_WIDTH-1:0]    s_axis_tx_resp_dest,
    input  wire                             s_axis_tx_resp_valid,
    output wire                             s_axis_tx_resp_ready,


    // Descriptor request output
    output wire [QUEUE_INDEX_WIDTH-1:0]     m_axis_desc_req_queue,
    output wire [WQE_WIDTH-1:0]             m_axis_desc_req_wqe,
    ...
    // Descriptor data input (AXI stream)
    ...
    output wire [AXIS_TX_ID_WIDTH-1:0]      m_axis_tx_desc_id,//queue
...
reg s_axis_tx_resp_ready_reg = 1'b0, s_axis_tx_resp_ready_next;
...
reg [WQE_WIDTH-1:0] m_axis_desc_req_wqe_reg = {WQE_WIDTH{1'b0}}, m_axis_desc_req_wqe_next;
...
reg [2:0] inc_active_desc_req;
reg dec_active_desc_req_1;
reg dec_active_desc_req_2;

reg [DESC_TABLE_SIZE-1:0] desc_table_active = 0;// 描述符是否激活
reg [DESC_TABLE_SIZE-1:0] desc_table_invalid = 0;// 描述符是否无效
reg [DESC_TABLE_SIZE-1:0] desc_table_desc_fetched = 0;// 描述符是否已获取
reg [DESC_TABLE_SIZE-1:0] desc_table_data_fetched = 0;// 数据是否已获取
(* ram_style = "distributed", ramstyle = "no_rw_check, mlab" *)
reg desc_table_tx_done_a[DESC_TABLE_SIZE-1:0];// 传输完成状态A
(* ram_style = "distributed", ramstyle = "no_rw_check, mlab" *)
reg desc_table_tx_done_b[DESC_TABLE_SIZE-1:0];// 传输完成状态B
reg [DESC_TABLE_SIZE-1:0] desc_table_cpl_write_done = 0;// 完成写入状态
// (* ram_style = "distributed", ramstyle = "no_rw_check, mlab" *)
// reg [REQ_TAG_WIDTH-1:0] desc_table_tag[DESC_TABLE_SIZE-1:0];
(* ram_style = "distributed", ramstyle = "no_rw_check, mlab" *)
reg [QUEUE_INDEX_WIDTH-1:0] desc_table_queue[DESC_TABLE_SIZE-1:0];// 队列索引
(* ram_style = "distributed", ramstyle = "no_rw_check, mlab" *)
reg [QUEUE_PTR_WIDTH-1:0] desc_table_queue_ptr[DESC_TABLE_SIZE-1:0];// 队列指针
(* ram_style = "distributed", ramstyle = "no_rw_check, mlab" *)
reg [CQN_WIDTH-1:0] desc_table_cpl_queue[DESC_TABLE_SIZE-1:0];// 完成队列
(* ram_style = "distributed", ramstyle = "no_rw_check, mlab" *)
reg [AXIS_TX_DEST_WIDTH-1:0] desc_table_dest[DESC_TABLE_SIZE-1:0];// 目标
(* ram_style = "distributed", ramstyle = "no_rw_check, mlab" *)
reg [6:0] desc_table_csum_start[DESC_TABLE_SIZE-1:0];// 校验和起始位置
(* ram_style = "distributed", ramstyle = "no_rw_check, mlab" *)
reg [7:0] desc_table_csum_offset[DESC_TABLE_SIZE-1:0];// 校验和偏移量
(* ram_style = "distributed", ramstyle = "no_rw_check, mlab" *)
reg desc_table_csum_enable[DESC_TABLE_SIZE-1:0];// 校验和使能
(* ram_style = "distributed", ramstyle = "no_rw_check, mlab" *)
reg [DMA_CLIENT_LEN_WIDTH-1:0] desc_table_len[DESC_TABLE_SIZE-1:0];// 长度
(* ram_style = "distributed", ramstyle = "no_rw_check, mlab" *)
reg [CL_TX_BUFFER_SIZE+1-1:0] desc_table_buf_ptr[DESC_TABLE_SIZE-1:0];// 缓冲区指针
(* ram_style = "distributed", ramstyle = "no_rw_check, mlab" *)
reg [PTP_TS_WIDTH-1:0] desc_table_ptp_ts[DESC_TABLE_SIZE-1:0];// PTP时间戳
(* ram_style = "distributed", ramstyle = "no_rw_check, mlab" *)
reg desc_table_read_commit[DESC_TABLE_SIZE-1:0];// 读取提交
(* ram_style = "distributed", ramstyle = "no_rw_check, mlab" *)
reg [DESC_TABLE_DMA_OP_COUNT_WIDTH-1:0] desc_table_read_count_start[DESC_TABLE_SIZE-1:0];// 读取计数开始
(* ram_style = "distributed", ramstyle = "no_rw_check, mlab" *)
reg [DESC_TABLE_DMA_OP_COUNT_WIDTH-1:0] desc_table_read_count_finish[DESC_TABLE_SIZE-1:0];// 读取计数完成

(* ram_style = "distributed", ramstyle = "no_rw_check, mlab" *)
reg [WQE_WIDTH-1:0] desc_table_wqe[DESC_TABLE_SIZE-1:0];


reg [CL_DESC_TABLE_SIZE+1-1:0] desc_table_start_ptr_reg = 0;// 开始指针
reg [QUEUE_INDEX_WIDTH-1:0] desc_table_start_queue;// 开始队列
reg [WQE_WIDTH-1:0] desc_table_start_wqe;// 
reg [AXIS_TX_DEST_WIDTH-1:0] desc_table_start_dest;// 开始目标
reg desc_table_start_en;
reg [CL_DESC_TABLE_SIZE-1:0] desc_table_dequeue_ptr;// 出队指针
reg [QUEUE_PTR_WIDTH-1:0] desc_table_dequeue_queue_ptr;// 出队队列指针
reg [CQN_WIDTH-1:0] desc_table_dequeue_cpl_queue;// 出队完成队列
reg desc_table_dequeue_invalid;// 出队无效
reg desc_table_dequeue_en;
reg [CL_DESC_TABLE_SIZE-1:0] desc_table_desc_ctrl_ptr;// 描述符控制指针
reg [CL_TX_BUFFER_SIZE+1-1:0] desc_table_desc_ctrl_buf_ptr;// 描述符控制缓冲区指针
reg [6:0] desc_table_desc_ctrl_csum_start;// 描述符控制校验和起始位置
reg [7:0] desc_table_desc_ctrl_csum_offset;// 描述符控制校验和偏移量
reg desc_table_desc_ctrl_csum_enable;// 描述符控制校验和使能
reg desc_table_desc_ctrl_en;// 描述符控制使能
reg [CL_DESC_TABLE_SIZE-1:0] desc_table_desc_fetched_ptr;// 描述符已获取指针
reg [DMA_CLIENT_LEN_WIDTH-1:0] desc_table_desc_fetched_len;// 描述符已获取长度
reg desc_table_desc_fetched_en;// 描述符已获取使能
reg [CL_DESC_TABLE_SIZE-1:0] desc_table_data_fetched_ptr;// 数据已获取指针
reg desc_table_data_fetched_en;// 数据已获取使能
reg [CL_DESC_TABLE_SIZE+1-1:0] desc_table_tx_start_ptr_reg = 0;// 传输开始指针
...
reg [CL_DESC_TABLE_SIZE+1-1:0] desc_table_cpl_enqueue_start_ptr_reg = 0;// 完成队列开始指针
reg desc_table_cpl_enqueue_start_en;// 完成队列开始使能
reg [CL_DESC_TABLE_SIZE-1:0] desc_table_cpl_write_done_ptr;// 完成写入指针
reg desc_table_cpl_write_done_en;// 完成写入使能
reg [CL_DESC_TABLE_SIZE+1-1:0] desc_table_finish_ptr_reg = 0;// 完成指针
reg desc_table_finish_en;// 完成使能
reg [CL_DESC_TABLE_SIZE+1-1:0] desc_table_read_start_ptr;// 读取开始指针
reg desc_table_read_start_commit;// 读取开始提交
reg desc_table_read_start_init;// 读取开始初始化
reg desc_table_read_start_en;// 读取开始使能
reg [CL_DESC_TABLE_SIZE+1-1:0] desc_table_read_finish_ptr;// 读取完成指针
reg desc_table_read_finish_en;// 读取完成使能
...
assign s_axis_tx_resp_ready = s_axis_tx_resp_ready_reg;
...
assign m_axis_desc_req_wqe = m_axis_desc_req_wqe_reg;
...
initial begin
    $dumpfile("tx_core.fst");
    // $dumpvars(0, desc_table_tx_done_a[2]);
    // $dumpvars(0, desc_table_tx_done_b[2]);
    // $dumpvars(0, desc_table_tx_done_a[1]);
    // $dumpvars(0, desc_table_tx_done_b[1]);
end

reg IF_IN = 1'b0;   
reg tx_req_pending_reg = 1'b0,tx_req_pending_next;

always @* begin
    m_axis_tx_req_valid_next = 1'b0;
    s_axis_tx_resp_ready_next = 1'b0;
    ...
    m_axis_desc_req_wqe_next = m_axis_desc_req_wqe_reg;
    ...
    desc_table_start_wqe = s_axis_tx_resp_wqe;
    desc_table_start_queue = s_axis_tx_resp_queue;
    ...
    desc_table_desc_fetched_ptr = s_axis_desc_tid & DESC_PTR_MASK;//todo:tid x?
    ...
    // descriptor fetch
    // request descriptor fetch to pifo
    if(enable && !tx_req_pending_reg && active_desc_req_count_reg < MAX_DESC_REQ && !desc_table_active[desc_table_start_ptr_reg & DESC_PTR_MASK] && ($unsigned(desc_table_start_ptr_reg - desc_table_finish_ptr_reg) < DESC_TABLE_SIZE)) begin
        m_axis_tx_req_valid_next = 1'b1;
    end

    if (m_axis_tx_req_valid && m_axis_tx_req_ready) begin
        m_axis_tx_req_valid_next = 1'b0;
        tx_req_pending_next = 1'b1;
    end
    
    // wait transmit request from pifo then send descriptor fetch to desc_fetch
    s_axis_tx_resp_ready_next = enable && tx_req_pending_reg &&
                        active_desc_req_count_reg < MAX_DESC_REQ 
                        ...
                        && (!m_axis_desc_req_valid || m_axis_desc_req_ready);
                        
    if (s_axis_tx_resp_ready && s_axis_tx_resp_valid) begin 
        s_axis_tx_resp_ready_next = 1'b0;
 
        // store in descriptor table
        desc_table_start_queue = s_axis_tx_resp_queue;
        desc_table_start_dest = s_axis_tx_resp_dest;
        desc_table_start_en = 1'b1;

        // initiate descriptor fetch
        m_axis_desc_req_queue_next = s_axis_tx_resp_queue;
        m_axis_desc_req_wqe_next = s_axis_tx_resp_wqe;
        m_axis_desc_req_tag_next = desc_table_start_ptr_reg & DESC_PTR_MASK;
        m_axis_desc_req_valid_next = 1'b1;

        inc_active_desc_req = s_axis_tx_resp_wqe;

        tx_req_pending_next = 1'b0;
    end

    // wait descriptor fetch status from desc_fetch
    if (s_axis_desc_req_status_valid) begin 

        // update entry in descriptor table
        desc_table_dequeue_ptr = s_axis_desc_req_status_tag & DESC_PTR_MASK;
        desc_table_dequeue_queue_ptr = s_axis_desc_req_status_ptr;//消费者指针
        desc_table_dequeue_cpl_queue = s_axis_desc_req_status_cpl;//完成队列号，分配的,有问题
        desc_table_dequeue_invalid = 1'b0;
        desc_table_dequeue_en = 1'b1;

        if (s_axis_desc_req_status_error || s_axis_desc_req_status_empty) begin //和队列管理相关，生产者指针是否等于消费者指针
    ...
    if (desc_table_active[desc_table_tx_start_ptr_reg & DESC_PTR_MASK] && desc_table_tx_start_ptr_reg != desc_table_start_ptr_reg) begin
        ...
        //end else if (desc_table_data_fetched[desc_table_tx_start_ptr_reg & DESC_PTR_MASK] && !m_axis_tx_desc_valid && (!m_axis_tx_csum_cmd_valid || !TX_CHECKSUM_ENABLE)) begin
        end else if (desc_table_desc_fetched[desc_table_tx_start_ptr_reg & DESC_PTR_MASK] && desc_table_read_commit[desc_table_tx_start_ptr_reg & DESC_PTR_MASK] 
                    && (desc_table_read_count_start[desc_table_tx_start_ptr_reg & DESC_PTR_MASK] == desc_table_read_count_finish[desc_table_tx_start_ptr_reg & DESC_PTR_MASK]) //?
                    && !m_axis_tx_desc_valid && (!m_axis_tx_csum_cmd_valid || !TX_CHECKSUM_ENABLE)) begin
            // update entry in descriptor table
            desc_table_tx_start_en = 1'b1;
            ...
            m_axis_tx_desc_valid_next = 1'b1;//发起读数据请求
    ...
    // transmit DMA done 传输DMA完成
    // wait for transmit DMA completion; free buffer space 等待传输DMA完成；释放缓冲区空间
    if (s_axis_tx_desc_status_valid) begin//读数据状态返回 
        // update entry in descriptor table 更新描述符中的表目
        desc_table_tx_dma_finish_ptr = s_axis_tx_desc_status_tag;//?
        desc_table_tx_dma_finish_en = 1'b1;//?

        // update read pointer 更新读指针
...
//todo:
    // transmit done 传输完成
    // wait for transmit completion; store PTP timestamp 等待传输完成；存储PTP时间戳
    s_axis_tx_cpl_ready_next = 1'b1;
    if (s_axis_tx_cpl_valid && s_axis_tx_cpl_tag[TX_TAG_WIDTH-1]) begin//s_axis_tx_cpl_tag?
        desc_table_tx_finish_ptr = s_axis_tx_cpl_tag;
        desc_table_tx_finish_ts = s_axis_tx_cpl_ts;
        desc_table_tx_finish_en = 1'b1;
    end

    // finish transmit; start completion enqueue 完成传输；开始完成队列
    if (desc_table_active[desc_table_cpl_enqueue_start_ptr_reg & DESC_PTR_MASK] 
        ...
        && desc_table_cpl_enqueue_start_ptr_reg != desc_table_tx_start_ptr_reg) begin
        if (desc_table_invalid[desc_table_cpl_enqueue_start_ptr_reg & DESC_PTR_MASK]) begin
            // invalid entry; skip 无效的表目；跳过
            desc_table_cpl_enqueue_start_en = 1'b1;
        end else if (desc_table_tx_done_a[desc_table_cpl_enqueue_start_ptr_reg & DESC_PTR_MASK] != desc_table_tx_done_b[desc_table_cpl_enqueue_start_ptr_reg & DESC_PTR_MASK] 
                && !m_axis_cpl_req_valid_next) begin
            // update entry in descriptor table 更新描述符中的表目
            desc_table_cpl_enqueue_start_en = 1'b1;
    ...
    // start completion write 开始完成写
    // wait for queue query response 等待队列查询响应
    ...
    if (desc_table_active[desc_table_finish_ptr_reg & DESC_PTR_MASK] 
        && desc_table_finish_ptr_reg != desc_table_start_ptr_reg 
        && desc_table_finish_ptr_reg != desc_table_cpl_enqueue_start_ptr_reg 
        && !finish_tx_req_status_valid_reg) begin

        if (desc_table_invalid[desc_table_finish_ptr_reg & DESC_PTR_MASK]) begin
            // invalidate entry in descriptor table 使描述符中的表目无效
            desc_table_finish_en = 1'b1;
        end else if (desc_table_cpl_write_done[desc_table_finish_ptr_reg & DESC_PTR_MASK]) begin
            // invalidate entry in descriptor table 使描述符中的表目无效
            desc_table_finish_en = 1'b1;

            // return transmit request completion 返回传输请求完成
    ...
    if (finish_tx_req_status_valid_next) begin
        finish_tx_req_status_valid_next = 1'b0;
    end
end

integer j;
localparam MAX_WQE = 4;
always @(posedge clk) begin

    m_axis_tx_req_valid_reg <= m_axis_tx_req_valid_next;
    s_axis_tx_resp_ready_reg <= s_axis_tx_resp_ready_next;
    ...
    m_axis_desc_req_wqe_reg <= m_axis_desc_req_wqe_next;
    ...
    if (desc_table_start_en) begin
        ...
        for (j = 0; j < MAX_WQE; j = j + 1) begin
            if (j < s_axis_tx_resp_wqe) begin
                desc_table_active[(desc_table_start_ptr_reg + j) & DESC_PTR_MASK] <= 1'b1;
                desc_table_invalid[(desc_table_start_ptr_reg + j) & DESC_PTR_MASK] <= 1'b0;
                desc_table_desc_fetched[(desc_table_start_ptr_reg + j) & DESC_PTR_MASK] <= 1'b0;
                desc_table_data_fetched[(desc_table_start_ptr_reg + j) & DESC_PTR_MASK] <= 1'b0;
                desc_table_tx_done_a[(desc_table_start_ptr_reg + j) & DESC_PTR_MASK] <= desc_table_tx_done_b[(desc_table_start_ptr_reg + j) & DESC_PTR_MASK];
                desc_table_cpl_write_done[(desc_table_start_ptr_reg + j) & DESC_PTR_MASK] <= 1'b0;
                desc_table_queue[(desc_table_start_ptr_reg + j) & DESC_PTR_MASK] <= desc_table_start_queue;
                desc_table_dest[(desc_table_start_ptr_reg + j) & DESC_PTR_MASK] <= desc_table_start_dest;
            end
        end
        desc_table_wqe[(desc_table_start_ptr_reg) & DESC_PTR_MASK] <= s_axis_tx_resp_wqe;
        desc_table_start_ptr_reg <= desc_table_start_ptr_reg + s_axis_tx_resp_wqe;
    end

    if (desc_table_dequeue_en) begin
        for (j = 0; j < MAX_WQE; j = j + 1) begin
            if (j < desc_table_wqe[desc_table_dequeue_ptr & DESC_PTR_MASK]) begin
                desc_table_queue_ptr[(desc_table_dequeue_ptr + j) & DESC_PTR_MASK] <= desc_table_dequeue_queue_ptr;
                desc_table_cpl_queue[(desc_table_dequeue_ptr + j) & DESC_PTR_MASK] <= desc_table_dequeue_cpl_queue;
            end
        end
        if (desc_table_dequeue_invalid) begin
            for (j = 0; j < MAX_WQE; j = j + 1) begin
                if (j < desc_table_wqe[desc_table_dequeue_ptr & DESC_PTR_MASK]) begin
                    desc_table_invalid[(desc_table_dequeue_ptr + j) & DESC_PTR_MASK] <= 1'b1;
                end 
            end
        end
    end

    if (desc_table_desc_ctrl_en) begin // 描述符控制使能
    ...
    if (desc_table_desc_fetched_en) begin // 描述符已获取使能
    ...
    if (desc_table_data_fetched_en) begin // 数据已获取使能
        desc_table_data_fetched[desc_table_data_fetched_ptr & DESC_PTR_MASK] <= 1'b1;
    end

    if (desc_table_tx_start_en) begin // 传输开始使能
    ...
    if (desc_table_tx_finish_en) begin // 传输完成使能
        if (PTP_TS_ENABLE) begin
    ...
    if (desc_table_read_finish_en) begin // 读取完成使能
