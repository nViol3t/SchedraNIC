...
from cocotb.triggers import Event, Edge, RisingEdge, Timer
...
class Txq:
    ...
    async def tx_transmit(self,data,rank,csum_cmd):
        ...
        assert not self.tx_info[index]
        self.tx_info[index] = pkt
        ...
        # write descriptors
        seg = min(length-offset, 42) if self.desc_block_size > 1 else length-offset
        struct.pack_into("<HHLQ", self.buf, index*self.stride, 0, csum_cmd, seg, ptr+offset if seg else 0)
        offset += seg
        for k in range(1, self.desc_block_size):
            seg = min(length-offset, 4096) if k < self.desc_block_size-1 else length-offset
            struct.pack_into("<4xLQ", self.buf, index*self.stride+k*MQNIC_DESC_SIZE, seg, ptr+offset if seg else 0)
            offset += seg

        self.prod_ptr += 1
        await self.write_prod_ptr(rank,1)#wqe=1 
    ...
    async def write_prod_ptr(self, rank, wqe):
        # Publish the new producer pointer before ringing the scheduler doorbell.
        # Otherwise the queue manager can schedule the doorbell while the queue still
        # appears empty and drop the last WQE of a burst.
        await self.hw_regs.write_dword(MQNIC_QUEUE_CTRL_STATUS_REG, MQNIC_QUEUE_CMD_SET_PROD_PTR | (self.prod_ptr & MQNIC_QUEUE_PTR_MASK))
        await self.hw_regs.write_dword(MQNIC_QUEUE_RANK_WQE, rank << 3 | wqe )

    # def free_desc(self, index):
    #     pkt = self.tx_info[index]
    #     if pkt is None:
    #         self.driver.log.error(f"free_desc called twice or missing packet: index={index}, ring.cons_ptr={self.cons_ptr}, ring.prod_ptr={self.prod_ptr}")
    #     else:
...
class SchedulerBlock:
    ...
    async def init(self):
        ...
        self.log.info("Scheduler no use")
...
class Interface:
    ...
    async def open(self):
        for k in range(self.rxq_res.get_count()):
            cq = Cq(self)
            await cq.open(self.eq[k % len(self.eq)], 1024)#change
            await cq.arm()
            rxq = Rxq(self)
            await rxq.open(cq, 8192, 4) #change
            await rxq.enable()
            self.rxq.append(rxq)
        ...
        for k in range(self.txq_res.get_count()):
            ...
            await cq.arm()
            txq = Txq(self)
            await txq.open(cq, 512, 4) #change
            await txq.enable()
            self.txq.append(txq)
    ...
    async def start_xmit(self, skb, tx_ring=None, csum_start=None, csum_offset=None, rank=0xfff):
        ...
        while True:
            ...
            # wait for space
            ring.clean_event.clear()#避免旧的唤醒残留
            await ring.clean_event.wait()#挂起协程，直到有人 ring.clean_event.set()

        csum_cmd = 0
        if csum_start is not None and csum_offset is not None:
            csum_cmd = 0x8000 | (csum_offset << 8) | csum_start

        await ring.tx_transmit(data,rank,csum_cmd)
