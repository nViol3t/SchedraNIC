...
async def run_test_nic(dut):
    ...
    #         await interface.start_xmit(data, 0, rank = 0x100)
    ...
    #     await tb.driver.interfaces[0].start_xmit(test_pkt2.build(), 0, 34, 6, 0x100)
    # else:
    #     await tb.driver.interfaces[0].start_xmit(test_pkt.build(), 0, rank = 0x100)
    ...
    #     await tb.driver.interfaces[0].start_xmit(data, k % len(tb.driver.interfaces[0].txq), rank = 0x100)

    #     pkt = await tb.driver.interfaces[0].recv()

    #     tb.log.info("Packet: %s")
    #     if tb.driver.interfaces[0].if_feature_rx_csum:
    #         assert pkt.rx_checksum == ~scapy.utils.checksum(bytes(pkt.data[14:])) & 0xffff
    #     assert pkt.queue == k
    ...
    #             await tb.driver.interfaces[0].start_xmit(test_pkt2.build(), k % len(tb.driver.interfaces[0].txq), 34, 6, rank = 0x100)
    #         else:
    #             await tb.driver.interfaces[0].start_xmit(test_pkt.build(), k % len(tb.driver.interfaces[0].txq), rank = 0x100)
    ...
    #     await tb.driver.interfaces[0].start_xmit(p, 0, rank = 0x100)
    ...
    count = 2000
    num_q = len(tb.driver.interfaces[0].txq)
    ...
    # 发送
    for k in range(count):
        await tb.driver.interfaces[0].start_xmit(
            pkts[k],
            k % num_q,
            rank=0x100
        )
    ...
    # 接收检查
    for rx_idx in range(count):

        pkt = await tb.driver.interfaces[0].recv()
        data = bytes(pkt.data)

        # 从payload恢复 k (mod 256)
        k_mod = data[0]

        # 推导发送队列
        inferred_queue = k_mod % num_q

        # 期望队列（按发送顺序）
        expected_queue = rx_idx % num_q

        tb.log.info(
            "RX idx=%d expected_q=%d inferred_q=%d len=%d",
            rx_idx,
            expected_queue,
            inferred_queue,
            len(data)
        ...
        # 队列顺序检查
        if inferred_queue != expected_queue:
            tb.log.error(
                "QUEUE ORDER ERROR: expected_q=%d inferred_q=%d",
                expected_queue,
                inferred_queue
            )

        # payload完整性检查
        exp_pkt = bytearray([(x + k_mod) % 256 for x in range(60)])

        if data != exp_pkt:
            tb.log.error("DATA MISMATCH queue=%d", inferred_queue)
            tb.log.error("exp_head=%r", bytes(exp_pkt[:16]))
            tb.log.error("got_head=%r", bytes(data[:16]))

        if tb.driver.interfaces[0].if_feature_rx_csum:
            assert pkt.rx_checksum == ~scapy.utils.checksum(bytes(data[14:])) & 0xffff
    ...
    # for k in range(count):
    #     await tb.driver.interfaces[0].start_xmit(pkts[k], k % len(tb.driver.interfaces[0].txq),rank = 0x100)
    ...
    #         await tb.driver.interfaces[0].start_xmit(pkts[p], p % len(tb.driver.interfaces[0].txq),rank = 0x100)
